...
namespace Vfi.Ui.Mvc.Vfi.Areas.Factory.Controllers {
    public class MachineController : Controller {
        ...
        List<MachineStateModel> GetMachineStateList() {
            ...
            try {
                using (var vfi = new vfiContext()) {
                    var machineStates = vfi.MachineStates.OrderBy(ms => !ms.Active).ThenBy(ms => ms.StateCode);
                    var i = 0;
                    foreach (var machineState in machineStates) {
                        var entity = new MachineStateModel {
            ...
            try {
                ...
                using (var vfi = new vfiContext()) {
                    ...
                    if (model == null) {
                        ...
                        vfi.MachineStates.Add(machineState);
                        vfi.SaveChanges();
                    }
                    else {
        ...
        public ActionResult UpdateMachineState(MachineStateModel updateModel, string colorPick) {
            try {
                ...
                using (var vfi = new vfiContext()) {
                    ...
                    if (existModel != null) {
                        var model =
                             vfi.MachineStates.FirstOrDefault(
                                 ms =>
        ...
        List<MachineStateDetailModel> GetMachineStateListDetail(int stateId) {
            ...
            try {
                using (var vfi = new vfiContext()) {
                    var machineStates = vfi.MachineStateDetails.Where(ms => ms.StateId == stateId).OrderBy(ms => !ms.Active).ThenBy(ms => ms.StateCode);
                    int i = 0;
                    foreach (var machineState in machineStates) {
