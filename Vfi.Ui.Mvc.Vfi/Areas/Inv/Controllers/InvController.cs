    ...
    public class InvController : Controller {
        ...
        public ActionResult AutoCompletedMaterialInventory(string text, int? materialTypeId) {
            ...
            using (var vfi = new tammaContext()) {
                if (materialTypeId != null && materialTypeId != 0)
                    model = vfi.Materials.Where(m => m.MaterialTypeId == materialTypeId).Select(m => m.MaterialCode).ToList();
            ...
            try {
                using (var vfi = new tammaContext()) {
                    ...
                    while (startDate < toDate) {
                        ...
                        var import = importsProduction.Where(pip => pip.Date >= startDate && pip.Date < endDate);
                        if (import.Any()) {
        ...
        private List<CncFormDetailModel> GetCncFormDetailCardModel(int productId, DateTime fromDate, DateTime toDate, string lotNumber) {
            ...
            using (var vfi = new tammaContext()) {
                ...
                if (!lotNumber.ToLower().Equals("all")) {
                    importCncs = importCncs.Where(pip => pip.ProductInvCode.Equals(lotNumber)).ToList();
                }
            ...
            try {
                using (var vfi = new tammaContext()) {
                    ...
                    foreach (var materialInventory in materialInvs) {
                        ...
                        var destroy = destroys.Where(ed => ed.MaterialInvId == entity.MaterialInventoryId);
                        if (destroy.Any())
                            entity.ExportDestroy = destroy.Sum(ed => ed.Quantity);
                        var productionsById = productions.Where(id => id.MaterialInvId == entity.MaterialInventoryId);
                        if (productionsById.Any()) {
                            entity.ExportUse = productionsById.Sum(id => id.MaterialUse);
                        }
                        var destroysById = destroysOnMachine.Where(md => md.MaterialInvId == entity.MaterialInventoryId);
                        if (destroysById.Any()) {
                            entity.DestroyOnMachine = destroysById.Sum(ed => ed.Quantity);
                        }
                        var materialOnMachines =
                            vfi.MaterialInvOnMachines.Where(mim => mim.MaterialInvId == entity.MaterialInventoryId);
            ...
            try {
                using (var vfi = new tammaContext()) {
                    var importSx1s = (from sxd in vfi.ImportFormSX1Detail
                                      ...
                                      where sxd.ImportFormSX1.ImportDate.Day == day &&
                                            sxd.ImportFormSX1.ImportDate.Month == month &&
                                            sxd.ImportFormSX1.ImportDate.Year == year &&
                                            (productId == 0 || sxd.ProductId == productId) &&
                                            (materialInvId == 0 || sxd.MaterialInvId == materialInvId) &&
                    ...
                    if (!lotNumber.ToLower().Equals("all")) {
                        importSx1s = importSx1s.Where(id => id.LotNumber.Equals(lotNumber)).ToList();
                    }
                        ...
                        if (lastTrack != null) {
                            ...
                            var totalInvs =
                                vfi.MaterialInventories.Where(
                                    mi => mi.MaterialId == entity.MaterialId && Math.Round(mi.TotalQty) > 0);
                            if (totalInvs.Any())
                                entity.MaterialAllInvTotal = totalInvs.Sum(mi => mi.TotalQty);
                            ...
                            if (materialInvOnmachines.Any())
                                entity.MaterialInvOnMachine = materialInvOnmachines.Sum(mim => mim.TotalQuantity);
                            var waitingUse =
                                useTemp.Where(
                                    mud =>
                            ...
                            if (lastTrack.TrackUpMaterials.Any()) {
                                ...
                                //entity.MaterialId = materiaInv.MaterialId;
                                //entity.MaterialInventoryCode = MyUtilities.Material.GetMaterialInvDesignNo(materiaInv);
                                ...
                                entity.MaterialInvTotal =
                                    lastTrack.TrackUpMaterials.LastOrDefault().MaterialInventory.TotalQty;
        ...
        List<SoLieuTongHopSanPham> GetProductInventoryInfo(int productId, int quantity, string lotNumber) {
            ...
            try {
                using (var vfi = new tammaContext()) {
                    ...
                    if (!lotNumber.Equals("All"))
                        productInvs = productInvs.Where(pi => pi.LotNumber.Equals(lotNumber));
        ...
        public ActionResult SelectQcInvManagement(
            ...
            try {
                ...
                using (var vfi = new tammaContext()) {
                    ...
                    var production2Ids = MyUtilities.Warehouse.GetWarehouseIdProduction2_ALL();
                    var platingInvIds = MyUtilities.Warehouse.GetWarehouseIds_Plating();
                    var qcInvIds = MyUtilities.Warehouse.GetWarehouseIdQc();
                    var processingInvIds = MyUtilities.Warehouse.GetWarehouseId_ReProcessing();
                    foreach (var productId in productIds) {
                        var product = products.FirstOrDefault(p => p.ProductId == productId);
                        if (product == null) continue;
            ...
            try {
                ...
                using (var vfi = new tammaContext()) {
                    ...
                    foreach (var productInv in productInvs) {
                        ...
                        periods = periods.Where(pip => pip.PeriodDate < from).ToList();
                        entity.EarlyQuantity = periods.Sum(pip => pip.PeriodQuantity);

                        periodById =
                            periodById.Where(pip => pip.PeriodDate >= from).ToList();
                        periods =
                            periodById.Where(
                                pip =>
                                pip.Transaction.WarehouseIssueId == MyUtilities.Warehouse.Production1 &&
